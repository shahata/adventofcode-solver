...
let table = ` Weapons:    Cost  Damage  Armor
              Dagger        8     4       0
              Shortsword   10     5       0
              Warhammer    25     6       0
              Longsword    40     7       0
...
export function day21(input) {
  let items = table.split('\n')
                   .filter(x => !x.match(/^\s*$/))
  ...
  let [hit, damage, armor] = input.match(/\d+/g).map(x => parseInt(x, 10));
  let boss = {hit, damage, armor};
  let options = [];
  ...
  let win = options.filter(x => Math.ceil(100 / Math.max(1, boss.damage - x.armor)) >= Math.ceil(boss.hit / Math.max(1, x.damage - boss.armor)));
  let lose = options.filter(x => Math.ceil(100 / Math.max(1, boss.damage - x.armor)) < Math.ceil(boss.hit / Math.max(1, x.damage - boss.armor)));

  let part1 = win.shift().cost;
  let part2 = lose.pop().cost;
  return [part1, part2];
}
